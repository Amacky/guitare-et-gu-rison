 ...
 * @since 1.0.0
 */
function lswss_slashes_deep($data = array(), $flag = false) {
  
    if($flag != true) {
        $data = lswss_nohtml_kses($data);
    }
...
function lswss_nohtml_kses($data = array()) {
  
  if ( is_array($data) ) {
    
    $data = array_map('lswss_nohtml_kses', $data);
...
function lswss_get_post_types() {
	...
	$include_post_types = apply_filters('lswss_gallery_support', array(LSWSS_POST_TYPE));
	$custom_post_types = array_merge($custom_post_types, (array)$include_post_types);
	
	// Exclude some post type
	$exclude_post_types = apply_filters('lswss_gallery_support', array('attachment'));
